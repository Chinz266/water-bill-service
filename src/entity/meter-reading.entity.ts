...
export class MeterReadingEntity {
  ...
  // 🌟 **บ้าน**ของการจดครั้งนี้ ใครเป็นคนเลือก — entry_method ตอบเรื่อง "เลข" ไม่ใช่ "บ้าน"
  //    ระบบออกบิลให้เองได้เมื่อจับคู่บ้านจากเลขมิเตอร์ได้แบบ high แต่ก่อนมีคอลัมน์นี้
  //    พอบิลออกไปแล้วไม่เหลือร่องรอยเลยว่าใบไหนระบบเลือกเอง คำถาม "ที่ auto อยู่ผิดกี่ %"
  //    จึงตอบไม่ได้ และการขยับเกณฑ์ออกบิลอัตโนมัติกลายเป็นการเดา
  //
  //    ⚠️ **หลักฐานย้อนหลังเท่านั้น ห้ามเอาไปเป็นเงื่อนไขของด่านใด ๆ** — ค่ามาจาก client
  //       ซึ่งปลอมได้ ต่างจาก read_confidence ที่หลังบ้านคำนวณเอง ด่านที่ตัดสินเรื่องเงิน
  //       ต้องยืนบนค่าที่หลังบ้านคิดเองเท่านั้น (ดู db/migrate-match-provenance.sql)
  //
  //    NULL = การจดที่ไม่ได้มาจากหน้าสแกน หรือเกิดก่อนมีคอลัมน์นี้
  @Column({
    type: 'enum',
  ...
  // 🌟 ระบบมั่นใจแค่ไหนตอนชี้บ้านหลังนี้ — ค่าตรงกับ ScanRow.matchConfidence ของหน้าเว็บ
  //    เก็บคู่กับ matched_by เพราะ 'system' เฉย ๆ ตอบไม่ได้ว่าเป็นใบที่ไหลออกไปเอง
  //    (high) หรือใบที่คนกดยืนยันตามที่ระบบเสนอ (medium) ซึ่งเป็นคนละความเสี่ยงกัน
  @Column({
    type: 'enum',
