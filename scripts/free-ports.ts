 ...
 * และ start-vision.ts ก็แตกลูกเป็น python.exe เวลาปิด terminal หรือถูก kill แบบ
...
function findListeners(port: number): number[] {
  try {
...
for (const port of PORTS) {
  for (const pid of findListeners(port)) {
    ...
    if (kill(pid)) {
      ...
      console.warn(
        `⚠️  ปิด PID ${pid} บนพอร์ต ${port} ไม่สำเร็จ — อาจต้องรัน terminal แบบ Administrator`,
      );
    }
  }
}
