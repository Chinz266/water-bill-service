...
/**
 * รูปมิเตอร์ที่ยังออกบิลไม่ได้ — คิวรอ Admin ตัดสินทีหลัง
 *
 * เกิดจากสามเคสที่หน้างานแก้เองไม่ได้:
 ...
 *   3. รู้แล้วว่าบ้านไหน แต่ด่านหลังบ้านตีกลับ (หน่วยพุ่ง / เลขต่ำกว่าเดือนก่อน /
 *      จดสลับตัวในกลุ่มมิเตอร์ที่ติดกัน) แล้วคนหน้างานไม่กล้ากดยืนยันเอง
 *      → `members_id` + `blocked_code` มีค่า ส่วนเคส 1-2 เป็น NULL ทั้งคู่
 *
...
export class UnassignedReadingEntity {
  ...
  /**
   * บ้านที่คนหน้างานเลือกไว้แล้ว — NULL = ยังไม่รู้ว่าของบ้านไหน (เคสกำพร้าแท้ ๆ)
   *
   * มีค่าเมื่อรูปเข้าคิวเพราะ **ด่านตีกลับ** ไม่ใช่เพราะไม่รู้ว่าเป็นของใคร
   * คนที่มาตรวจทีหลังจะได้ไม่ต้องไล่หาบ้านใหม่ทั้งที่คนหน้างานรู้อยู่แล้วว่ายืนอยู่หน้าบ้านไหน
   *
   * ⚠️ ยังเป็นแค่ "ข้อเสนอ" ไม่ใช่ข้อสรุป — คนตรวจเปลี่ยนบ้านตอนกดจับคู่ได้เสมอ
   *    เพราะเหตุผลหนึ่งที่รูปมาอยู่ตรงนี้คือ "อาจจดสลับบ้าน" ซึ่งแปลว่าบ้านที่เลือกไว้อาจผิด
   */
  @Column({ type: 'int', nullable: true })
  members_id!: number | null;

  /**
   * รหัสด่านที่ตีกลับตอนพยายามออกบิล (เช่น HIGH_USAGE, CLUSTER_SEQUENCE_MISMATCH)
   *
   * เก็บ **รหัส** ไม่ใช่แค่ข้อความ เพราะหน้าตรวจต้องรู้ว่าจะขึ้นช่องยืนยันตัวไหนให้กด
   * (หน่วยพุ่ง → confirm_high_usage, เลขต่ำกว่าเดือนก่อน → confirm_meter_reset ฯลฯ)
   * ถ้าเก็บแต่ข้อความ หน้าตรวจจะต้องมาเดาจากคำในประโยค ซึ่งพังทันทีที่มีใครไปแก้คำ
   */
  @Column({ type: 'varchar', length: 45, nullable: true })
  blocked_code!: string | null;

  /** ข้อความที่ด่านตอบกลับไปตอนนั้น — เก็บไว้ให้คนตรวจเห็นสิ่งเดียวกับที่คนหน้างานเห็น */
